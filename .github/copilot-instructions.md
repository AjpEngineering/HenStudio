...
- When creating or restoring database SQL files, use the actual repository path with a leading underscore: `_HenStudioDatabase\Tables\...` rather than `HenStudioDatabase`. Ensure files are written to `C:\_AJP\git\HenStudio\_HenStudioDatabase\Tables\...`.
